...
var paddleI;					//reference to the players paddle
...
function demoLoop()
{
	...
	paddleI.simulateplay(activeball[0]);
...
{
	for (var i = 0; i < activeball.length; i++) {
		if (activeball[i].touching(paddleI))
			activeball[i].hitsprite(paddleI);
	}
}
/*
...
function paddleCollisions()
{
	for (var i = 0; i < activeball.length; i++) {
		if (activeball[i].touching(paddleI))
		{
			activeball[i].hitpaddle(paddleI);
		}
	}
	//now check for pickups
	for (var i = 0; i < pickups.length; i++)
	{
		if (!pickups[i].dead && pickups[i].touching(paddleI))
		{
			pickups[i].kill();
			pickups[i].pickupAction(paddleI);
		}
	}	
